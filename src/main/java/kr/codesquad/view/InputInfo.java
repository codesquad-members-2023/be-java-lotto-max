...
import static kr.codesquad.util.Conversion.storeWinningNums;
...
public class InputInfo {
    ...
    public static Set<Integer> pickWinningNumber(BufferedReader br, Validation validation) throws IOException {
        System.out.println("당첨 번호를 입력해 주세요.");
        String winningNums = br.readLine();
        while (!validation.isValidNums(storeWinningNums(winningNums))) {
            System.out.println("잘못 입력하셨습니다. 1~45 중 중복되지 않는 숫자 6개를 입력해주세요. \n숫자는 쉼표로 구분해주세요.");
            winningNums = br.readLine();
        }
        return storeWinningNums(winningNums);
    }
